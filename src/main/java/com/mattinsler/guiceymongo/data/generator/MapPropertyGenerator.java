	...
	public void createWrapperMethod(Appendable builder, MapProperty property, int indentCount) throws IOException {
		...
		if (keyType instanceof UserEnumType) {
			...
			if (valueType instanceof UserEnumType)
				s +=				"map.put($p.mapKeyType$.valueOf(com.mattinsler.guiceymongo.util.DBObjectUtil.decodeKey(key)), $p.mapValueType$.valueOf((String)obj.get(key)));\n";
			else if (valueType instanceof UserDataType)
				s +=				"map.put($p.mapKeyType$.valueOf(com.mattinsler.guiceymongo.util.DBObjectUtil.decodeKey(key)), $p.mapValueType$.wrap((com.mongodb.DBObject)obj.get(key)));\n";
			else if (valueType instanceof ListType) {
				Type itemType = ((ListType)valueType).getItemType();
					...
					s +=
										...
										"if (l != null && l instanceof java.util.List<?>) {\n" +
											...
											"for ($i.javaType$ listValue : (java.util.List<$i.javaBoxedType$>)l)\n" +
												"list.add(listValue);\n" +
											"map.put($p.mapKeyType$.valueOf(com.mattinsler.guiceymongo.util.DBObjectUtil.decodeKey(key)), list);\n" +
										"}";
				}
			} else if (valueType instanceof SetType) {
				...
				if (itemType instanceof PrimitiveType) {
					s +=
										...
										"if (l != null && l instanceof java.util.List<?>) {\n" +
											...
											"for ($i.javaType$ listValue : (java.util.List<$i.javaBoxedType$>)l)\n" +
												"set.add(listValue);\n" +
											"map.put($p.mapKeyType$.valueOf(com.mattinsler.guiceymongo.util.DBObjectUtil.decodeKey(key)), set);\n" +
										"}";
				}
			} else
				s +=				"map.put($p.mapKeyType$.valueOf(com.mattinsler.guiceymongo.util.DBObjectUtil.decodeKey(key)), ($p.mapValueType$)obj.get(key));\n";
			s +=				"} catch (Exception e) {\n" +
								"}\n";
		} else if (PrimitiveType.StringType.equals(keyType)) {
			if (valueType instanceof UserEnumType) {
				s += 			"try {\n" +
									"map.put(com.mattinsler.guiceymongo.util.DBObjectUtil.decodeKey(key), $p.mapValueType$.valueOf((String)obj.get(key)));\n" +
								"} catch (Exception e) {\n" +
								"}\n";
			} else if (valueType instanceof UserDataType)
				s += 			"map.put(com.mattinsler.guiceymongo.util.DBObjectUtil.decodeKey(key), $p.mapValueType$.wrap((com.mongodb.DBObject)obj.get(key)));\n";
			else if (valueType instanceof ListType) {
				Type itemType = ((ListType)valueType).getItemType();
				template.setAttribute("i", itemType);
				if (itemType instanceof PrimitiveType) {
					s +=
								...
								"if (l != null && l instanceof java.util.List<?>) {\n" +
									...
									"for ($i.javaType$ listValue : (java.util.List<$i.javaBoxedType$>)l)\n" +
										"list.add(listValue);\n" +
									"map.put(com.mattinsler.guiceymongo.util.DBObjectUtil.decodeKey(key), list);\n" +
								"}\n";
				}
			} else if (valueType instanceof SetType) {
				Type itemType = ((SetType)valueType).getItemType();
				...
				if (itemType instanceof PrimitiveType) {
					s +=
								...
								"if (l != null && l instanceof java.util.List<?>) {\n" +
									...
									"for ($i.javaType$ listValue : (java.util.List<$i.javaBoxedType$>)l)\n" +
										"set.add(listValue);\n" +
									"map.put(com.mattinsler.guiceymongo.util.DBObjectUtil.decodeKey(key), set);\n" +
								"}\n";
				}
			} else
				s += 			"map.put(com.mattinsler.guiceymongo.util.DBObjectUtil.decodeKey(key), ($p.mapValueType$)obj.get(key));\n";
		} else
...
//		builder.putView(key, MailMessageEntity.View.newBuilder(value.getView(key)));
	...
	public void createBuilderBuild(Appendable builder, MapProperty property, int indentCount) throws IOException {
		...
		if (valueType instanceof UserEnumType) {
			...
			if (keyType instanceof UserEnumType)
				s +=	"map.put(com.mattinsler.guiceymongo.util.DBObjectUtil.encodeKey(value.getKey().name()), value.getValue().name());\n";
			else
				s +=	"map.put(com.mattinsler.guiceymongo.util.DBObjectUtil.encodeKey(value.getKey()), value.getValue().name());\n";
			s +=
			...
			if (keyType instanceof UserEnumType)
				s +=	"map.put(com.mattinsler.guiceymongo.util.DBObjectUtil.encodeKey(value.getKey().name()), (($p.builderMapValueType$)value.getValue()).build());\n";
			else
				s +=	"map.put(com.mattinsler.guiceymongo.util.DBObjectUtil.encodeKey(value.getKey()), (($p.builderMapValueType$)value.getValue()).build());\n";
			s +=
					"dbObject.put($p.keyName$, map);\n" +
				"}\n";
		} else if (valueType instanceof SetType) {
			...
			if (itemType instanceof PrimitiveType) {
				...
				if (keyType instanceof UserEnumType)
					s +=
						"map.put(com.mattinsler.guiceymongo.util.DBObjectUtil.encodeKey(value.getKey().name()), list);\n";
				else
					s +=
						"map.put(com.mattinsler.guiceymongo.util.DBObjectUtil.encodeKey(value.getKey()), list);\n";
				s +=
					"}\n" +
					"dbObject.put($p.keyName$, map);\n" +
				"}\n";
