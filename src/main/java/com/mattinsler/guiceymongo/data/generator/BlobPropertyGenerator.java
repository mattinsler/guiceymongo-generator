...
//		appendIndent(builder, indentCount).append("public static final com.mattinsler.guiceymongo.data.BlobProperty<").append(property.getEnclosingType().getCanonicalJavaType()).append(", ").append(property.getType().getCanonicalJavaType()).append("> ").append(property.getKeyName()).append(" = com.mattinsler.guiceymongo.data.BlobProperty.create(\"").append(keyValue).append("\");\n");
		...
		StringTemplate template = new StringTemplate(
				...
				"private boolean load$p.camelCaseName$() {\n" +
					"if ($p.memberVariableName$ == null && $p.memberVariableName$Bucket != null && $p.memberVariableName$Identity != null) {\n" +
		        		"com.mattinsler.guiceymongo.GuiceyBucket guiceyBucket = com.mattinsler.guiceymongo.guice.GuiceyMongoUtil.getGuiceyBucket($p.memberVariableName$Bucket);\n" +
		                "if (guiceyBucket == null)\n" +
	...
	public void createBuilderBuild(Appendable builder, BlobProperty property, int indentCount) throws IOException {
		StringTemplate template = new StringTemplate(
				"if ($p.memberVariableName$ != null && $p.memberVariableName$.size() > 0 && $p.memberVariableName$Identity == null) {\n" +
					...
					"com.mattinsler.guiceymongo.GuiceyBucket guicey$p.camelCaseName$Bucket = com.mattinsler.guiceymongo.guice.GuiceyMongoUtil.getGuiceyBucket($p.memberVariableName$Bucket);\n" +
