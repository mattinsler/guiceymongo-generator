 ...
 */
public class GuiceyDataGeneratorTest {
    ...
    private static final Pattern REGEX_IDENTITY = Pattern.compile("String IdentityKey = \"_id\"");
    ...
    @Test
    public void shouldChangeIdentityKey() {
        GENERATOR.generate(SCHEMA_PERSON);
        ...
        try {
			final String contents = FileUtils.readFileToString(person);
			Assert.assertTrue(REGEX_IDENTITY.matcher(contents).find());
		} catch (IOException e) {
			e.printStackTrace();
			Assert.fail("Could not read generated file.");
		}
